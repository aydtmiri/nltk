            ...
            try:
                for child in children(node):
                    if child not in traversed:
                        ...
                        warnings.warn('Discarded redundant search for {0} at depth {1}'.format(child, depth + 1),
                                      stacklevel=2)
            except TypeError:
                pass
...
def acyclic_depth_first(tree, children=iter, depth=-1, cut_mark=None, traversed=None):
    ...
    >>> import nltkma
    >>> from nltkma.util import acyclic_depth_first as acyclic_tree
    >>> wn=nltkma.corpus.wordnet
    >>> from pprint import pprint
...
def acyclic_branches_depth_first(tree, children=iter, depth=-1, cut_mark=None, traversed=None):
    ...
    >>> import nltkma
    >>> from nltkma.util import acyclic_branches_depth_first as tree
    >>> wn=nltkma.corpus.wordnet
    >>> from pprint import pprint
        ...
        try:
            for child in children(tree):
                if child not in traversed:
                    ...
                    out_tree += [
                        acyclic_branches_depth_first(child, children, depth - 1, cut_mark, traversed.union({child}))]
                else:
                    warnings.warn('Discarded redundant search for {0} at depth {1}'.format(child, depth - 1),
                                  stacklevel=3)
                    if cut_mark:
...
def unweighted_minimum_spanning_tree(tree, children=iter):
    ...
    >>> import nltkma
    >>> from nltkma.util import unweighted_minimum_spanning_tree as mst
    >>> wn=nltkma.corpus.wordnet
    >>> from pprint import pprint
...
def flatten(*args):
    ...
    Flatten a list.
        ...
        >>> from nltkma.util import flatten
...
def bigrams(sequence, **kwargs):
    ...
    For example:
        ...
        >>> from nltkma.util import bigrams
...
def trigrams(sequence, **kwargs):
    ...
    For example:
        ...
        >>> from nltkma.util import trigrams
