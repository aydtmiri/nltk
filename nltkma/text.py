...
from nltkma.lm import MLE
from nltkma.lm.preprocessing import padded_everygram_pipeline
from nltkma.probability import FreqDist
from nltkma.probability import ConditionalFreqDist as CFD
from nltkma.util import tokenwrap, LazyConcatenation
from nltkma.metrics import f_measure, BigramAssocMeasures
from nltkma.collocations import BigramCollocationFinder
from nltkma.tokenize import sent_tokenize
...
class ConcordanceIndex:
    ...
    def find_concordance(self, pivot_tokens, target_tokens, span, context, allow_self_reference):
        ...
        b = BigramCollocationFinder.from_words(pivot_tokens, target_tokens, self._tokens, span, allow_self_reference, )
        ...
        for collocation in b.pos:
            for i in range(len(b.pos[collocation])):
                ...
                query_word = self._tokens[pos]
                # Find the context of query word.
                # i- context = contex left span
                context_left_exists = True
                context_right_exists = True

                left_span = self._tokens[max(0, pos - span[0]): pos]

                try:
                    right_span = self._tokens[pos + 1: pos + span[1] + 1]
                except IndexError:
                    right_span = self._tokens[pos + 1: self._tokens[len(self._tokens) - 1]]
                    context_right_exists = False

                # get additional context
                if pos - span[0] > 0:
                    try:
                        left_context = self._tokens[max(0, pos - span[0] - context[0]): pos - span[0]]
                    except IndexError:
                        left_context = self._tokens[max(0, self._tokens[0]): pos - span[0]]
                else:
                    left_context = []
                if pos + span[1] < len(self._tokens) - 1:
                    try:
                        right_context = self._tokens[pos + 1 + span[1]: pos + span[1] + 1 + context[1]]
                    except IndexError:
                        right_context = self._tokens[pos + span[1] + 1: self._tokens[len(self._tokens) - 1]]
                else:
                    right_context = []
    ...
    def find_concordance_old(self, word, width=80):
        """
        Find all concordance lines given the query word.
...
class TokenSearcher:
    ...
    def findall(self, regexp):
        ...
        >>> from nltkma.text import TokenSearcher
        >>> print('hack'); from nltkma.book import text1, text5, text9
        hack...
        >>> text5.findall("<.*><.*><bro>")
...
class Text:
    ...
    >>> from nltkma.text import Text
    >>> moby = Text(nltkma.corpus.gutenberg.words('melville-moby_dick.txt'))
    ...
    def concordance_list(self, pivot_tokens, target_tokens, span, context, allow_self_reference):
        """
        ...
        return self._concordance_index.find_concordance(pivot_tokens, target_tokens, span, context,
    ...
    def collocation_list(self, num=20, window_size=2):
        ...
        if not (
            ...
            from nltkma.corpus import stopwords
    ...
    def dispersion_plot(self, words):
        ...
        """
        from nltkma.draw import dispersion_plot
...
class TextCollection(Text):
    ...
    >>> import nltkma.corpus
    >>> from nltkma.text import TextCollection
    >>> print('hack'); from nltkma.book import text1, text2, text3
    hack...
    >>> gutenberg = TextCollection(nltkma.corpus.gutenberg)
